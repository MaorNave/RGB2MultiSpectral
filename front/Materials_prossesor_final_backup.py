...
class Materials_prossesor():
    ...
    def parallel_clu_rgb(frame_tup):
        ...
        try:
            flat_clu_bgr_img = frame_clu_rgb.reshape(-1, frame_clu_rgb.shape[2])
        except:
            print(frame_num)
            print(full_clu_path)
        ...
        unique_clu_rgb_list = []
        for seg_bgr in unique_seg_list:
            indices = np.where(flat_seg_img == seg_bgr)
            rgb_list = flat_clu_bgr_img[indices]
            unique_rgb_list = np.unique(rgb_list, axis=0)
            segment_name = objects_seg_rgb_data_global[
                Materials_prossesor.array_to_string_defulter_rgb_v(Materials_prossesor, str(seg_bgr))]
        ...
        for clu_rgb in full_clu_unique_rgb:
            ...
            indices = np.where(np.all(flat_clu_bgr_img == clu_rgb, axis=1))
            rgb_list = flat_rgb_img[indices]
            if len(rgb_list) == 0:
                continue
            mean_bgr = np.mean(np.array(rgb_list), axis=0)
            mean_dict[str_clu_rgb] = (mean_bgr, len(rgb_list))
    ...
    def cluster_rgb(self, main_clu_rgb_path, output_folder, frames_list):
        ...
        pool = mp.Pool(6)
        results = list(pool.map(self.bin_image, [(os.path.join(main_clu_rgb_path, frame), cluster_labels, os.path.join(save_folder, frame)) for frame in frames_list]))
        pool.close()
