...
class Materials():
    ...
    def clu_rgb_to_materials_labels_rgb_swir_dict(clu_rgb, obj):
        ...
        :return: 12D vector reflectance by the chosen material to this base_rgb
        """
    ...
    def cube_generator(item_tup):
        ...
        for obj, obj_seg_indices in objects_seg_rgb_dict_relevant.items():
            ...
            unique_clu_rgb = np.unique(clu_rgb_on_seg_list.reshape(-1, 3), axis=0)
    ...
    def cubes_generator(self):
        ...
        for test_folder in tqdm(test_folders_list):
            ...
            test_output_mat_dict_folder = os.path.join(self.path['test'], 'cubes_generator', test_folder, 'Max_score/cubes_materials_dict')
            output_folder = os.path.join(self.path['results'], 'cubes_generator',test_folder, 'Max_score/cubes')
            rgb_mask_output_folder = os.path.join(self.path['results'], 'cubes_generator',test_folder, 'Max_score/masks/rgb_masks')
            heatmap_mask_folder = os.path.join(self.path['results'], 'cubes_generator',test_folder, 'Max_score/masks/heatmap_mask')
            psedo_rgb_mask_output_folder = os.path.join(self.path['results'], 'cubes_generator',test_folder, 'Max_score/masks/psedo_rgb_masks')
            material_type_mask_folder = os.path.join(self.path['results'], 'cubes_generator',test_folder, 'Max_score/masks/material_type_mask')
            ...
            seg_frames_list = np.sort(os.listdir(main_seg_path))[:30]
            clu_frames_list = np.sort(os.listdir(main_clu_rgb_path))[:30]
            ...
            p = mp.Pool(4, initializer=self.worker_init_cubes_gen, initargs=(global_args_list,))

            results = p.map(Materials.cube_generator, [arg_tup for arg_tup in items])

            # Materials.cube_generator(items[0])




    @staticmethod
    def parallel_raffle_cubes(data_tup):
    ...
    def cubes_generator_raffle(self):
            ...
            """
            ##confifure special override table for spacial cases
            global max_cubes_list
            global override_dict_relavent
            global main_seg_path
            global main_max_cubes_path
            global output_folder
            global seg_dict_relevant
            global materials_labels_rgb_swir_dict
            global objects_clu_rgb_mat
            global objects_seg_rgb_dict
            global objects_distance_base_rgb_mat
            global test_output_mat_dict_folder
            global output_folder
            global seg_mask_output_folder
            global rgb_mask_output_folder
            global instance_seg_frame_output_folder
            global seg_table_output_folder
            global materials_mask_folder
            global class_balance_vector_folder
            global material_type_mask_folder
            global distance_heatmap_map_folder
            global heatmap_mask_folder

            # ray.init()

            test_folder_list = self.config['cubes_generator']['test_folders_list']
            main_masks_path = self.config['cubes_generator']['main_masks_path']
            material_version_folder = self.config['cubes_generator']['materials_version_folder']

            seg_table = pd.read_excel(self.config['cubes_generator']['seg_table'])
            ##change relevant scene id##
            seg_table_relevant_scene_id = self.config['cubes_generator']['seg_table_scene_name']
            raffle_folder_name = self.config['cubes_generator']['raffle_folder']
            seg_dict_relevant, relevant_seg_table = self.seg_table_to_dict(seg_table, seg_table_relevant_scene_id)

            for test_folder in test_folder_list:

                main_seg_path = os.path.join(main_masks_path, test_folder, 'Semantic_segmentation')

                main_max_cubes_path = os.path.join(self.path['results'], material_version_folder, test_folder,
                                                      'Max_score/cubes')
                override_table = pd.read_csv(os.path.join(self.path['deploy'], material_version_folder, test_folder,
                                                          self.config['cubes_generator']['override_table_name']))
                override_dict_relavent = self.override_table_to_dict(override_table)
                # main_max_cubes_path = os.path.join(main_cubes_output_folder, test_folder, algo_version, 'Ref/Max_Score')
                materials_labels_rgb_swir_dict = self.yaml_loader(
                    os.path.join(self.path['deploy'], material_version_folder,test_folder, self.config['cubes_generator']['materials_rgb_swir']))

                objects_clu_rgb_mat = self.pickle_loader(
                    os.path.join(self.path['deploy'], material_version_folder, test_folder,
                                 self.config['cubes_generator']['objects_base_rgb_scores_mat']))
                objects_distance_base_rgb_mat = self.pickle_loader(
                    os.path.join(self.path['deploy'], material_version_folder, test_folder,
                                 self.config['cubes_generator']['objects_base_rgb_distance_mat']))
                objects_seg_rgb_dict = self.json_loader(
                    os.path.join(self.path['deploy'], material_version_folder, test_folder,
                                 self.config['cubes_generator']['obj_seg_rgb']))

                ###relevant output and test paths
                test_output_mat_dict_folder = os.path.join(self.path['test'], material_version_folder, test_folder, raffle_folder_name, 'cubes_materials_dict')
                output_folder = os.path.join(self.path['results'],material_version_folder ,test_folder, raffle_folder_name,'cubes')
                seg_mask_output_folder = os.path.join(self.path['results'],material_version_folder ,test_folder, raffle_folder_name,'masks/seg_masks')
                rgb_mask_output_folder = os.path.join(self.path['results'], material_version_folder,test_folder, raffle_folder_name,'masks/rgb_masks')
                materials_mask_folder = os.path.join(self.path['results'], material_version_folder,test_folder, raffle_folder_name,'masks/material_mask')
                instance_seg_frame_output_folder = os.path.join(self.path['results'], material_version_folder,test_folder,
                                                                raffle_folder_name,'masks/instance_seg_masks')
                class_balance_vector_folder = os.path.join(self.path['results'], material_version_folder,test_folder,
                                                           raffle_folder_name,'masks/class_balance_mask')
                material_type_mask_folder = os.path.join(self.path['results'], material_version_folder,test_folder,
                                                         raffle_folder_name,'masks/material_type_mask')
                seg_table_output_folder = os.path.join(self.path['results'],material_version_folder ,test_folder,
                                                       raffle_folder_name,'masks/seg_table')
                heatmap_mask_folder = os.path.join(self.path['results'], material_version_folder, test_folder,
                                                   raffle_folder_name,'masks/heatmap_mask')
                distance_heatmap_map_folder = os.path.join(self.path['results'], material_version_folder, test_folder,
                                                           raffle_folder_name,'masks/rgb_distance_heatmap_mask')
                output_path_list = [test_output_mat_dict_folder, output_folder, seg_mask_output_folder, rgb_mask_output_folder,
                                    instance_seg_frame_output_folder, seg_table_output_folder, materials_mask_folder \
                    , class_balance_vector_folder, material_type_mask_folder, distance_heatmap_map_folder, heatmap_mask_folder]

                for path in output_path_list:
                    self.folder_checker(path)

                #keep on checikng, with that try to work on linear reg also
                seg_frames_list = np.sort(os.listdir(main_seg_path))
                max_cubes_list = np.sort(os.listdir(main_max_cubes_path))
                ...
                print('start pooling')
                start = time.time()
                # pool = multiprocessing.Pool(processes=num_proccess)
                # pool.map(self.parallel_raffle_cubes, [(i,frame) for i, frame in enumerate(seg_frames_list)])
                self.parallel_raffle_cubes((0, seg_frames_list[0]))
                # futures = [self.parallel_raffle_cubes.remote((i,frame)) for i, frame in enumerate(seg_frames_list)]
                # results = [ray.get(i) for i in futures]
                end = time.time()
